...
import {
  ...
  Clock, TrendingUp, Users, BookOpen, Calendar, User,
  Heart, Lightbulb
} from 'lucide-react';
...
const GLOBAL_CSS = `
  ...
  * { font-family: 'Barlow', sans-serif; }
  ...
  @media (max-width: 768px) {
    ...
    .text-slate-300 { color: #d1d9e6 !important; }
    .text-slate-400 { color: #b8c5d6 !important; }
    p, li { letter-spacing: 0.01em; }
  }

  /* Mobile heading centre-alignment */
  @media (max-width: 1023px) {
    h1, h2, h3 { text-align: center; }
    article h1, article h2, article h3,
    .text-left-always, label,
    [role="table"] *, address { text-align: left !important; }
  }
  ...
  /* Logo hard-clamp */
  .ke-logo-wrap { width:36px!important; height:36px!important; min-width:36px!important; max-width:36px!important; overflow:hidden!important; flex-shrink:0!important; }
  .ke-logo-wrap img { width:100%!important; height:100%!important; object-fit:contain!important; display:block!important; }

  /* Animations — only those actually used in JSX */
...
const SKELETON_CSS = `
  ...
  .ske {
    ...
    animation: ske-wave 1.4s ease-in-out infinite;
    border-radius: 6px;
  }
  .ske-dark {
    ...
    background-size: 800px 100%;
    animation: ske-wave-dark 1.4s ease-in-out infinite;
    border-radius: 6px;
  }
  .ske-page-enter { animation: ske-fade-in 0.35s ease-out both; }
  .ske-delay-1{animation-delay:.05s}
  .ske-delay-2{animation-delay:.10s}
  .ske-delay-3{animation-delay:.15s}
  .ske-delay-4{animation-delay:.20s}
  .ske-delay-5{animation-delay:.25s}
  .ske-delay-6{animation-delay:.30s}
`;

/* ── Single reusable shimmer block ───────────────────────── */
const Ske=({cls='',dark=false})=><div className={`${dark?'ske-dark':'ske'} ${cls}`} aria-hidden="true"/>;

/* ── Shared dark nav-banner skeleton used on most pages ───── */
const DarkHeaderSke=({wide=false})=>(
  <div className="bg-[#0A1628] py-24 mb-10 sm:mb-16">
    <div className={`${wide?'max-w-7xl':'max-w-4xl'} mx-auto px-4 flex flex-col items-center gap-4`}>
      <Ske cls="w-20 h-3 ske-delay-1" dark/>
      <Ske cls="w-72 max-w-full h-8 ske-delay-2" dark/>
      <Ske cls="w-48 h-2 ske-delay-3" dark/>
      <Ske cls="w-80 max-w-full h-3 ske-delay-4" dark/>
    </div>
  </div>
);
...
const ProductCardSkeleton=memo(()=>(
  <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden" aria-hidden="true">
        <Ske cls="h-48 w-full rounded-none"/>
    <div className="p-6 space-y-3">
      <Ske cls="w-20 h-3 ske-delay-1"/>
      <Ske cls="w-full h-5 ske-delay-2"/>
      <Ske cls="w-4/5 h-4 ske-delay-2"/>
      <Ske cls="w-full h-3 ske-delay-3"/>
      <Ske cls="w-3/4 h-3 ske-delay-3"/>
      <div className="pt-4 border-t border-slate-100 flex gap-3">
        <Ske cls="flex-1 h-10 rounded-lg ske-delay-4"/>
        <Ske cls="flex-1 h-10 rounded-lg ske-delay-5"/>
      </div>
    </div>
  </div>
));
...
const ProductsPageSkeleton=memo(()=>(
  <main id="main-content" className="pb-20 bg-slate-50 min-h-screen ske-page-enter" aria-busy="true" aria-label="Loading products catalog">
        <DarkHeaderSke wide/>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col sm:flex-row gap-4 mb-8">
        <Ske cls="flex-1 h-12 rounded-xl"/>
        <Ske cls="w-36 h-12 rounded-xl ske-delay-1"/>
      </div>
      <div className="flex gap-2 mb-8 flex-wrap">
        {[80,110,90,120,100,95,85].map((w,i)=>(
          <Ske key={i} cls={`h-9 rounded-full ske-delay-${Math.min(i+1,6)}`} style={{width:w}}/>
        ))}
      </div>
...
const ProductDetailSkeleton=memo(()=>(
  <main id="main-content" className="pt-28 pb-20 bg-slate-50 min-h-screen ske-page-enter" aria-busy="true" aria-label="Loading product details">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex gap-2 items-center mb-8 pt-4">
        {[60,80,120].map((w,i)=>(
          <React.Fragment key={i}>
            <Ske cls="h-3 rounded" style={{width:w}}/>
            {i<2&&<Ske cls="w-3 h-3 rounded"/>}
          </React.Fragment>
        ))}
      </div>
      <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-lg">
        <div className="grid grid-cols-1 lg:grid-cols-12">
          <div className="lg:col-span-5 p-6 bg-slate-50 border-b lg:border-b-0 lg:border-r border-slate-200">
            <Ske cls="aspect-square w-full rounded-2xl mb-4"/>
            <div className="grid grid-cols-3 gap-2">
              {[0,1,2].map(i=><Ske key={i} cls={`aspect-square rounded-xl ske-delay-${i+1}`}/>)}
            </div>
          </div>
          <div className="lg:col-span-7 p-8 lg:p-12 space-y-4">
            <Ske cls="w-24 h-5 rounded-md"/>
            <Ske cls="w-full h-8 rounded ske-delay-1"/>
            <Ske cls="w-4/5 h-8 rounded ske-delay-2"/>
            <Ske cls="w-full h-3 rounded ske-delay-3"/>
            <Ske cls="w-3/4 h-3 rounded ske-delay-3"/>
            <div className="bg-slate-900 rounded-2xl p-5 space-y-3">
              <Ske cls="w-40 h-3 rounded ske-delay-1" dark/>
              <Ske cls="w-full h-3 rounded ske-delay-2" dark/>
              <Ske cls="w-5/6 h-3 rounded ske-delay-3" dark/>
            </div>
            <div className="flex gap-1">
              <Ske cls="w-32 h-9 rounded-t-lg"/>
              <Ske cls="w-32 h-9 rounded-t-lg ske-delay-1"/>
            </div>
            {Array.from({length:5}).map((_,i)=>(
              <div key={i} className={`flex gap-3 ske-delay-${Math.min(i+1,6)}`}>
                <Ske cls="w-2/5 h-8 rounded"/>
                <Ske cls="w-3/5 h-8 rounded"/>
              </div>
            ))}
            <div className="pt-7 border-t border-slate-200 flex gap-4 mt-4">
              <Ske cls="flex-1 h-14 rounded-xl ske-delay-4"/>
              <Ske cls="flex-1 h-14 rounded-xl ske-delay-5"/>
            </div>
          </div>
        </div>
      </div>
...
const BlogPageSkeleton=memo(()=>(
  <main id="main-content" className="pb-20 bg-white min-h-screen ske-page-enter" aria-busy="true" aria-label="Loading blog">
        <DarkHeaderSke wide/>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
        <Ske cls="aspect-video rounded-3xl"/>
        <div className="flex flex-col justify-center gap-4">
          <Ske cls="w-24 h-4 rounded ske-delay-1"/>
          <Ske cls="w-full h-7 rounded ske-delay-2"/>
          <Ske cls="w-4/5 h-7 rounded ske-delay-2"/>
          <Ske cls="w-full h-3 rounded ske-delay-3"/>
          <Ske cls="w-3/4 h-3 rounded ske-delay-4"/>
          <Ske cls="w-36 h-11 rounded-xl ske-delay-5"/>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[0,1,2].map(i=>(
          <div key={i} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            <Ske cls="aspect-video rounded-none"/>
            <div className="p-6 space-y-3">
              <div className="flex gap-2"><Ske cls="w-16 h-3 rounded ske-delay-1"/><Ske cls="w-20 h-3 rounded ske-delay-2"/></div>
              <Ske cls="w-full h-5 rounded ske-delay-2"/>
              <Ske cls="w-4/5 h-4 rounded ske-delay-3"/>
              <Ske cls="w-full h-3 rounded ske-delay-3"/>
              <Ske cls="w-28 h-9 rounded-lg ske-delay-4"/>
            </div>
          </div>
        ))}
      </div>
...
/* ── Blog post skeleton ───────────────────────────────────── */
const BlogPostSkeleton=memo(()=>(
  <main id="main-content" className="pb-20 bg-white min-h-screen ske-page-enter" aria-busy="true" aria-label="Loading article">
        <div className="bg-[#0A1628] py-24 mb-12">
      <div className="max-w-4xl mx-auto px-4 flex flex-col items-center gap-4">
        <div className="flex gap-2">
          {[60,80,70].map((w,i)=><Ske key={i} cls="h-6 rounded-full ske-delay-1" dark style={{width:w}}/>)}
        </div>
        <Ske cls="w-full h-9 rounded ske-delay-2" dark/>
        <Ske cls="w-4/5 h-7 rounded ske-delay-3" dark/>
        <div className="flex gap-6 mt-2">
          {[80,100,70].map((w,i)=><Ske key={i} cls="h-3 rounded ske-delay-4" dark style={{width:w}}/>)}
        </div>
      </div>
    </div>
    <div className="max-w-4xl mx-auto px-4 sm:px-6">
      <Ske cls="w-full aspect-video rounded-3xl mb-12"/>
      <div className="space-y-8">
        {Array.from({length:6}).map((_,i)=>(
          <div key={i} className={`space-y-3 ske-delay-${Math.min(i+1,6)}`}>
            {i%3===0&&<Ske cls="w-2/3 h-7 rounded"/>}
            <Ske cls="w-full h-3 rounded"/>
            <Ske cls="w-full h-3 rounded ske-delay-1"/>
            <Ske cls="w-4/5 h-3 rounded ske-delay-2"/>
            {i%2===0&&<Ske cls="w-3/4 h-3 rounded ske-delay-3"/>}
          </div>
        ))}
      </div>
      <div className="mt-16 bg-slate-50 border border-slate-200 rounded-2xl p-8 flex flex-col items-center gap-4">
        <Ske cls="w-48 h-6 rounded ske-delay-1"/>
        <Ske cls="w-64 h-3 rounded ske-delay-2"/>
        <Ske cls="w-40 h-12 rounded-xl ske-delay-3"/>
      </div>
    </div>
  </main>
));
...
const HomePageSkeleton=memo(()=>(
  <main id="main-content" className="min-h-screen bg-[#0A1628] ske-page-enter" aria-busy="true" aria-label="Loading">
        <section className="min-h-screen flex items-center pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-5">
            <div className="flex gap-3">
              <Ske cls="w-28 h-8 rounded-lg" dark/>
              <Ske cls="w-36 h-8 rounded-lg ske-delay-1" dark/>
            </div>
            <Ske cls="w-32 h-3 rounded ske-delay-1" dark/>
            <Ske cls="w-full h-12 rounded ske-delay-2" dark/>
            <Ske cls="w-4/5 h-12 rounded ske-delay-2" dark/>
            <Ske cls="w-3/5 h-12 rounded ske-delay-3" dark/>
            <Ske cls="w-full h-20 rounded-2xl ske-delay-3" dark/>
            <div className="flex gap-4 pt-2">
              <Ske cls="w-52 h-14 rounded-xl ske-delay-4" dark/>
              <Ske cls="w-48 h-14 rounded-xl ske-delay-5" dark/>
            </div>
          </div>
          <div className="hidden lg:flex flex-col gap-4">
            {[0,1,2,3].map(i=><Ske key={i} cls={`h-20 rounded-2xl ske-delay-${i+1}`} dark/>)}
          </div>
        </div>
      </div>
    </section>
...
const ServicesPageSkeleton=memo(()=>(
  <main id="main-content" className="pb-20 bg-white min-h-screen ske-page-enter" aria-busy="true" aria-label="Loading services">
        <DarkHeaderSke wide/>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-24">
      {[0,1,2].map(i=>(
        <div key={i} className={`flex flex-col md:flex-row gap-16 items-start ${i%2!==0?'md:flex-row-reverse':''}`}>
          <div className="md:w-2/5 w-full"><Ske cls="aspect-[4/3] rounded-3xl"/></div>
          <div className="md:w-3/5 w-full space-y-4">
            <Ske cls="w-24 h-3 rounded"/>
            <Ske cls="w-3/4 h-9 rounded ske-delay-1"/>
            <Ske cls="w-full h-4 rounded ske-delay-2"/>
            <Ske cls="w-5/6 h-4 rounded ske-delay-3"/>
            <div className="border border-slate-200 rounded-2xl overflow-hidden">
              <Ske cls="h-12 rounded-none"/>
              {[0,1,2,3,4].map(j=>(
                <div key={j} className={`h-12 bg-white border-t border-slate-100 px-6 flex items-center gap-3 ske-delay-${j+1}`}>
                  <Ske cls="w-4 h-4 rounded-full"/>
                  <Ske cls="flex-1 h-3 rounded"/>
                </div>
              ))}
            </div>
            <Ske cls="w-44 h-12 rounded-xl ske-delay-5"/>
          </div>
        </div>
      ))}
    </div>
...
/* ── Industries page skeleton ────────────────────────────── */
const IndustriesPageSkeleton=memo(()=>(
  <main id="main-content" className="pb-20 bg-white min-h-screen ske-page-enter" aria-busy="true" aria-label="Loading industries">
        <DarkHeaderSke wide/>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
      {[0,1,2].map(i=>(
        <div key={i} className={`flex flex-col md:flex-row gap-12 items-center ${i%2!==0?'md:flex-row-reverse':''}`}>
          <div className="md:w-1/2 w-full"><Ske cls="aspect-[4/3] rounded-3xl"/></div>
          <div className="md:w-1/2 w-full space-y-4">
            <Ske cls="w-16 h-16 rounded-2xl"/>
            <Ske cls="w-2/3 h-7 rounded ske-delay-1"/>
            <Ske cls="w-full h-3 rounded ske-delay-2"/>
            <Ske cls="w-5/6 h-3 rounded ske-delay-3"/>
            <div className="grid grid-cols-2 gap-2 pt-2">
              {[0,1,2,3].map(j=><Ske key={j} cls={`h-8 rounded-lg ske-delay-${j+1}`}/>)}
            </div>
          </div>
        </div>
      ))}
...
const ContactPageSkeleton=memo(()=>(
  <main id="main-content" className="pb-20 bg-slate-50 min-h-screen ske-page-enter" aria-busy="true" aria-label="Loading contact">
        <div className="bg-[#0A1628] py-20 mb-12">
      <div className="max-w-7xl mx-auto px-4 flex flex-col items-center gap-4">
        <Ske cls="w-48 h-9 rounded ske-delay-1" dark/>
        <Ske cls="w-64 h-3 rounded ske-delay-2" dark/>
        <div className="flex gap-3 mt-4">
          <Ske cls="w-36 h-11 rounded-xl ske-delay-3" dark/>
          <Ske cls="w-36 h-11 rounded-xl ske-delay-4" dark/>
        </div>
      </div>
    </div>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="space-y-4">
          {Array.from({length:7}).map((_,i)=>(
            <Ske key={i} cls={`h-14 rounded-xl ske-delay-${Math.min(i+1,6)}`}/>
          ))}
          <Ske cls="h-36 rounded-xl"/>
          <Ske cls="h-14 rounded-xl ske-delay-1"/>
        </div>
        <div className="space-y-5">
          {[0,1,2].map(i=>(
            <Ske key={i} cls={`h-28 rounded-2xl ske-delay-${i+1}`}/>
          ))}
          <Ske cls="h-96 rounded-3xl ske-delay-4"/>
        </div>
      </div>
    </div>
  </main>
...
/* ── About page skeleton ─────────────────────────────────── */
const AboutPageSkeleton=memo(()=>(
  <main id="main-content" className="pb-20 bg-white min-h-screen ske-page-enter" aria-busy="true" aria-label="Loading about">
        <div className="bg-[#0A1628] py-24">
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="space-y-5">
          <Ske cls="w-20 h-3 rounded" dark/>
          <Ske cls="w-full h-10 rounded ske-delay-1" dark/>
          <Ske cls="w-4/5 h-10 rounded ske-delay-2" dark/>
          <Ske cls="w-20 h-0.5 rounded ske-delay-2" dark/>
          <Ske cls="h-20 rounded-xl ske-delay-3" dark/>
          <Ske cls="w-full h-3 rounded ske-delay-4" dark/>
          <Ske cls="w-5/6 h-3 rounded ske-delay-4" dark/>
          <div className="flex flex-wrap gap-2 pt-2">
            {Array.from({length:6}).map((_,i)=>(
              <Ske key={i} cls={`h-7 w-28 rounded-full ske-delay-${Math.min(i+1,6)}`} dark/>
            ))}
          </div>
          <div className="flex gap-3 pt-2">
            <Ske cls="w-36 h-11 rounded-xl ske-delay-4" dark/>
            <Ske cls="w-44 h-11 rounded-xl ske-delay-5" dark/>
          </div>
        </div>
        <div className="hidden lg:flex flex-col gap-4">
          <Ske cls="h-56 rounded-3xl" dark/>
          <div className="grid grid-cols-2 gap-4">
            {[0,1,2,3].map(i=><Ske key={i} cls={`h-24 rounded-2xl ske-delay-${i+1}`} dark/>)}
          </div>
          <Ske cls="h-16 rounded-2xl ske-delay-5" dark/>
        </div>
      </div>
    </div>
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8">
        {[0,1,2,3].map(i=>(
          <div key={i} className="flex flex-col items-center gap-2">
            <div className={`w-20 h-8 rounded bg-blue-500/50 ske-delay-${i+1}`}/>
            <div className={`w-28 h-3 rounded bg-blue-500/40 ske-delay-${i+2}`}/>
          </div>
        ))}
      </div>
    </div>
  ...
  if (logoFile && !err) {
    return (
      <img
        ...
        loading="lazy"
        decoding="async"
        className="max-h-12 max-w-[140px] md:max-w-[160px] object-contain transition-all duration-300"
        onError={()=>setErr(true)}
      />
    );
  }
  ...
  return (
    <a href="#/" onClick={e=>{e.preventDefault();navigate('/');}} aria-label="Keshav Enterprises — Home"
      ...
      <div className="ke-logo-wrap" style={{width:36,height:36,minWidth:36,minHeight:36,maxWidth:36,maxHeight:36,overflow:'hidden',borderRadius:8,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center'}}>
        {!imgErr
          ? <img src="keshav-logo.png" alt="Keshav Enterprises" loading="lazy" decoding="async"
...
const MakeInIndiaBadge = memo(() => {
  ...
  return (
    <div className="inline-flex items-center space-x-3 glass px-4 py-2.5 rounded-lg shadow-xl w-fit" role="img" aria-label="Make In India — Vocal For Local">
      {!e?<img src="make-in-india.png" alt="Make In India" loading="lazy" width="32" height="32" className="h-8 object-contain" onError={()=>sE(true)}/>:<div className="w-8 h-8 bg-orange-500 rounded-full flex items-center justify-center"><Zap className="w-4 h-4 text-white" aria-hidden="true"/></div>}
...
const IndiaMartBadge = memo(() => {
  ...
  return (
    <a href={CONTACT_INFO.indiamart} target="_blank" rel="noopener noreferrer"
      ...
      {!e?<div className="h-8 bg-white rounded px-1.5 flex items-center justify-center"><img src="indiamart-logo.png" alt="IndiaMART" width="60" height="20" loading="lazy" decoding="async" className="h-5 object-contain" onError={()=>sE(true)}/></div>
  ...
  <footer className="bg-[#060E1C] text-slate-300 pt-20 pb-8 border-t border-blue-900/30" role="contentinfo">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
        <div>
          <div className="mb-6"><BrandLogo navigate={navigate}/></div>
...
/* ─────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────── */
/* ─────────────────────────────────────────────────────────────
   FEATURED PRODUCTS STRIP — touch/mouse draggable + nav arrows
   ───────────────────────────────────────────────────────────── */
const FeaturedProductsStrip = memo(({products, navigate}) => {
  ...
  const isPaused   = useRef(false);  // paused by hover / touch / drag
  const speed      = 0.7;            // px per frame — adjust for faster/slower auto-scroll
  ...
  const syncArrows = useCallback(() => {
    ...
    setCanRight(Math.ceil(el.scrollLeft + el.clientWidth) < el.scrollWidth - 4);
  }, []);

  /* ── Auto-marquee via rAF ── */
  const autoScroll = useCallback(() => {
    const el = stripRef.current; if (!el) return;
    if (!isPaused.current) {
      el.scrollLeft += speed;
      // Infinite loop: when we reach the halfway point (cloned cards start), jump back to 0
  ...
  const onTouchStart = useCallback(e => {
    isPaused.current   = true;
    isDragging.current = true;
    startX.current     = e.touches[0].pageX - stripRef.current.offsetLeft;
    scrollLeft.current = stripRef.current.scrollLeft;
  }, []);

  const onTouchMove = useCallback(e => {
    if (!isDragging.current) return;
    const x    = e.touches[0].pageX - stripRef.current.offsetLeft;
    const walk = (x - startX.current) * 1.2;
    stripRef.current.scrollLeft = scrollLeft.current - walk;
  }, []);
  ...
  const scroll = useCallback(dir => {
    isPaused.current = true;
    const cardW = stripRef.current?.querySelector('[data-card]')?.offsetWidth || 296;
    stripRef.current?.scrollBy({left: dir === 'left' ? -(cardW + 20) : (cardW + 20), behavior:'smooth'});
    // Resume after manual scroll settles
    setTimeout(() => { isPaused.current = false; }, 1200);
  }, []);

  /* ── Click guard — don't navigate if user dragged ── */
  const handleCardClick = useCallback((product) => {
    if (Math.abs(stripRef.current.scrollLeft - scrollLeft.current) > 6) return;
    navigate(`/product/${product.id}`);
  }, [navigate]);

  // Double the products array so the infinite-loop jump is seamless
  const doubled = useMemo(() => [...products, ...products], [products]);

  return (
    <div className="relative" onMouseEnter={pause} onMouseLeave={resume}>
      {/* Edge fades */}
      <div className="absolute left-0 top-0 w-10 md:w-20 h-full bg-gradient-to-r from-slate-50 to-transparent z-10 pointer-events-none"/>
      <div className="absolute right-0 top-0 w-10 md:w-20 h-full bg-gradient-to-l from-slate-50 to-transparent z-10 pointer-events-none"/>

      {/* Prev arrow */}
      <button onClick={() => scroll('left')} aria-label="Previous products"
        className={`absolute left-2 md:left-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white border border-slate-200 shadow-lg flex items-center justify-center text-slate-600 hover:text-blue-600 hover:border-blue-400 hover:shadow-xl transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${canLeft?'opacity-100 scale-100':'opacity-0 scale-90 pointer-events-none'}`}>
        <ChevronLeft className="w-5 h-5" aria-hidden="true"/>
      </button>

      {/* Scrollable strip — NO scroll-snap so auto-scroll is smooth */}
      <div
        ref={stripRef}
        className="flex gap-5 overflow-x-auto px-8 md:px-16 py-4 scrollbar-hide"
      ...
      >
        {doubled.map((product, idx) => (
          <div
            ...
            style={{flexShrink:0}}
            className="group flex flex-col w-64 md:w-72 bg-white border border-slate-200 rounded-2xl overflow-hidden hover:border-blue-400 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 cursor-pointer select-none"
          >
            <div className="h-44 bg-slate-50 border-b border-slate-100 flex items-center justify-center relative overflow-hidden">
              ...
              {product.images?.[0]
                ...
                : <div className="w-full h-full flex items-center justify-center pointer-events-none" aria-hidden="true">{getCategoryIcon(product.category)}</div>
              }
            </div>
      ...
      {/* Next arrow */}
      <button onClick={() => scroll('right')} aria-label="Next products"
        className={`absolute right-2 md:right-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white border border-slate-200 shadow-lg flex items-center justify-center text-slate-600 hover:text-blue-600 hover:border-blue-400 hover:shadow-xl transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${canRight?'opacity-100 scale-100':'opacity-0 scale-90 pointer-events-none'}`}>
    ...
    <main id="main-content" className="bg-white">
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden bg-[#0A1628]" aria-labelledby="hero-heading">
        ...
        <div className="absolute inset-0 z-0" aria-hidden="true">
          {/* Hero image — object-position ensures the turbine shows on mobile */}
          {!heroErr&&<img src="hero-background.png" alt="" width="1920" height="1080" fetchPriority="high" loading="eager" decoding="sync"
            className="absolute inset-0 w-full h-full"
            style={{objectFit:'cover', objectPosition:'60% center', opacity:0.85}}
            onError={()=>setHeroErr(true)}/>}
          {/* Mobile: bottom-heavy dark overlay so text on top is readable */}
          <div className="absolute inset-0 bg-gradient-to-t from-[#0A1628] via-[#0A1628]/60 to-[#0A1628]/30 lg:hidden"/>
          {/* Desktop: left gradient keeps text legible, image visible on right */}
          <div className="absolute inset-0 hidden lg:block bg-gradient-to-r from-[#0A1628] via-[#0A1628]/70 to-[#0A1628]/10"/>
          ...
          {/* Grid texture */}
          <div className="bg-grid absolute inset-0" style={{opacity:0.03}}/>
        </div>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 w-full pt-28 pb-24 sm:pt-32 flex flex-col lg:flex-row items-center justify-between gap-10 lg:gap-16">
          {/* Left — text content */}
          <div className="w-full lg:w-[58%]">
            <div className={`transition-all duration-1000 ${loaded?'opacity-100 translate-y-0':'opacity-0 translate-y-8'}`}>
              {/* Badges — row on mobile */}
              <div className="flex flex-wrap items-center gap-3 mb-6 sm:mb-8 justify-center lg:justify-start">
                <MakeInIndiaBadge/><IndiaMartBadge/>
              </div>
              {/* Eyebrow */}
              <div className="flex items-center gap-3 mb-4 sm:mb-5 justify-center lg:justify-start anim-slide-right delay-100">
                <div className="w-6 sm:w-10 h-0.5 bg-blue-400 rounded-full"/>
                <span className="font-display text-blue-300 font-black uppercase tracking-[0.18em] sm:tracking-[0.25em] text-xs sm:text-sm">Industrial Turbine Engineering</span>
              </div>
              {/* Main heading — centred on mobile */}
              <h1 id="hero-heading" className="font-display font-black text-white leading-[1.05] tracking-tight mb-5 sm:mb-6 anim-fade-up delay-200 text-center lg:text-left"
              ...
              {/* Descriptor box */}
              <div className="glass border-l-4 border-l-blue-400 p-4 sm:p-5 mb-8 sm:mb-10 rounded-r-2xl anim-fade-up delay-400 mx-auto lg:mx-0" style={{maxWidth:520}}>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 anim-fade-up delay-500">
                ...
                <a href={waMsg('Hi KESHAV ENTERPRISES, we have an emergency breakdown. Please assist immediately.')}
                  target="_blank" rel="noopener noreferrer"
                  className="glass text-white border border-white/25 px-6 sm:px-8 py-4 rounded-xl font-bold text-base sm:text-lg hover:bg-white/10 transition-all flex items-center justify-center hover:-translate-y-1 focus:outline-none focus-visible:ring-2 focus-visible:ring-white min-h-[54px] w-full sm:w-auto">
                  <LifeBuoy className="mr-2 sm:mr-3 w-5 h-5 text-cyan-300 shrink-0" aria-hidden="true"/> Emergency Breakdown
                </a>
              </div>
            </div>
          </div>
      ...
      {/* ── PRODUCTS MARQUEE ── */}
      <section className="py-20 bg-slate-50 border-b border-slate-200" aria-labelledby="featured-products-heading">
...
const AboutPage = ({navigate}) => {
  ...
  return (
    <main id="main-content" className="pb-20 bg-white min-h-screen">
      <SEOHead title="About Keshav Enterprises — 20+ Years of Turbine Engineering Excellence" description="Keshav Enterprises: 20+ years of industrial steam turbine engineering, reverse engineering and OEM-compatible spare parts manufacturing. Ex-Triveni, Siemens, BHEL engineers. Shamli, Uttar Pradesh, India."
      ...
      <div className="bg-[#0A1628] text-white relative overflow-hidden">
        {/* Background image — same as hero, same treatment */}
        <div className="absolute inset-0 z-0" aria-hidden="true">
          <img src="hero-background.png" alt="" width="1920" height="1080"
            className="absolute inset-0 w-full h-full"
            style={{objectFit:'cover', objectPosition:'60% center', opacity:0.35}}
            loading="lazy"/>
          {/* Mobile: top+bottom dark overlay */}
          <div className="absolute inset-0 bg-gradient-to-t from-[#0A1628] via-[#0A1628]/70 to-[#0A1628]/60 lg:hidden"/>
          {/* Desktop: heavier left so text is legible */}
          <div className="absolute inset-0 hidden lg:block bg-gradient-to-r from-[#0A1628]/95 via-[#0A1628]/80 to-[#0A1628]/40"/>
          <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-[#0A1628] to-transparent"/>
          <div className="bg-grid absolute inset-0" style={{opacity:0.03}}/>
        </div>
        {/* Radial glows */}
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-blue-600/8 rounded-full blur-[140px] pointer-events-none z-0" aria-hidden="true"/>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-24 pb-20 lg:py-28">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">

            {/* Left — headline + story text */}
            <div className="text-center lg:text-left">
              <div className="flex items-center gap-3 mb-5 justify-center lg:justify-start">
                <div className="w-6 sm:w-8 h-0.5 bg-blue-400 rounded-full"/>
                <span className="text-blue-300 font-black text-xs uppercase tracking-[0.25em]">Our Story</span>
              </div>
              <h1 className="font-display font-black text-white leading-[1.0] tracking-tight mb-6"
                style={{fontSize:'clamp(2.4rem,5.5vw,4.4rem)', textShadow:'0 2px 20px rgba(0,0,0,0.5)'}}>
                TWO DECADES OF<br/>
                <span className="text-gradient">PRECISION<br/>ENGINEERING</span>
              </h1>
              <div className="w-20 h-0.5 bg-blue-500 mb-8 rounded-full mx-auto lg:mx-0" aria-hidden="true"/>

              {/* Pull quote */}
              <div className="border-l-4 border-blue-500 pl-5 mb-8 glass rounded-r-xl py-4 pr-5 text-left">
                <p className="text-white text-base sm:text-lg font-bold leading-relaxed italic">
                  "Zero tolerance for plant downtime — that's not a slogan. It's the engineering standard every client holds us to."
                </p>
                <p className="text-blue-400 text-xs font-black uppercase tracking-widest mt-2">— Keshav Enterprises Engineering Leadership</p>
              </div>

              <p className="text-slate-200 text-sm sm:text-base leading-relaxed mb-6">
                From a specialist turbine maintenance outfit in Shamli, Uttar Pradesh — to a trusted, pan-India engineering partner for power plants, sugar mills, refineries, and process industries. Built on ex-OEM expertise from <strong className="text-white">Triveni, Siemens, BHEL, Man Turbo, KKK</strong> and ABB.
              </p>
              <p className="text-slate-300 text-sm leading-relaxed mb-8 hidden sm:block">
                We combine OEM-level technical knowledge with the speed and flexibility of a specialist contractor — delivering the right solution in days, not the 12–18 month lead times of original manufacturers.
              </p>

              {/* USP tags */}
              <div className="flex flex-wrap gap-2 mb-8 justify-center lg:justify-start">
                {['Ex-OEM Engineers','3D Laser Scanning','CMM Precision','24×7 Response','IndiaMART TrustSeal','Make In India'].map(tag=>(
                  <span key={tag} className="glass border border-blue-500/30 text-blue-200 text-[11px] font-black uppercase tracking-wider px-3 py-1.5 rounded-full">{tag}</span>
                ))}
              </div>

              <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start">
                <button onClick={()=>navigate('/services')}
                  className="bg-blue-600 text-white px-7 py-4 rounded-xl font-black text-sm hover:bg-blue-500 transition-all flex items-center justify-center gap-2 group shadow-[0_0_30px_rgba(30,111,255,0.3)] hover:-translate-y-0.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 min-h-[52px] w-full sm:w-auto">
                  Our Services <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" aria-hidden="true"/>
                </button>
                <button onClick={()=>navigate('/contact')}
                  className="glass text-white border border-white/20 px-7 py-4 rounded-xl font-black text-sm hover:bg-white/10 transition-all hover:-translate-y-0.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-white min-h-[52px] w-full sm:w-auto">
                  Contact Engineering Team
                </button>
              </div>
            </div>

            {/* Right — visual panel with stats */}
            <div className="hidden lg:flex flex-col gap-5" aria-hidden="true">
              {/* Large image card */}
              <div className="relative rounded-3xl overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(30,111,255,0.15)]" style={{height:'260px'}}>
                <img src="hero-background.png" alt="Industrial turbine facility — Keshav Enterprises" width="580" height="260" loading="lazy" decoding="async"
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <div className="flex items-end gap-4">
                    <div>
                      <div className="font-display font-black text-white text-5xl leading-none mb-1">20+</div>
                      <div className="text-blue-300 text-xs font-black uppercase tracking-[0.2em]">Years of Engineering Excellence</div>
                    </div>
                    <div className="ml-auto glass-dark px-4 py-2.5 rounded-xl border border-blue-500/30">
                      <div className="text-blue-400 font-black text-[10px] uppercase tracking-widest mb-0.5">Since</div>
                      <div className="text-white font-display font-black text-2xl">2000</div>
                    </div>
                  </div>
                </div>
                {/* Top badge */}
                <div className="absolute top-5 left-5 glass-dark px-3 py-1.5 rounded-lg border border-blue-500/30">
                  <span className="text-blue-300 text-[10px] font-black uppercase tracking-widest">Shamli, Uttar Pradesh · India</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  {val:'5 kW – 27 MW',label:'Turbine Range',color:'text-cyan-400',border:'border-cyan-500/25',bg:'bg-cyan-500/5'},
                  {val:'50 – 2,000 kg',label:'Dynamic Balancing',color:'text-yellow-400',border:'border-yellow-500/25',bg:'bg-yellow-500/5'},
                  {val:'ISO 4406:99',label:'Lube Oil Flushing',color:'text-green-400',border:'border-green-500/25',bg:'bg-green-500/5'},
                  {val:'4.3 / 5',label:'IndiaMART Rating',color:'text-orange-400',border:'border-orange-500/25',bg:'bg-orange-500/5'},
                ].map(({val,label,color,border,bg},i)=>(
                  <div key={i} className={`glass-dark rounded-2xl p-5 border ${border} ${bg} hover:-translate-y-0.5 transition-transform`}>
                    <div className={`font-display font-black text-xl mb-1 ${color}`}>{val}</div>
                    <div className="text-slate-400 text-[10px] font-black uppercase tracking-wider">{label}</div>
                  </div>
                ))}
              </div>

              {/* Certification bar */}
              <div className="glass-dark rounded-2xl p-5 border border-white/8 flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-slate-400 text-xs mt-0.5">Verified supplier · 50+ buyer reviews · 4.3/5 rating</div>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[
              ...
              <div key={i}>
                <div className="font-display font-black text-white text-3xl md:text-4xl mb-0.5">{stat}</div>
...
const ServicesPage = ({navigate}) => (
  <main id="main-content" className="pt-24 pb-20 bg-white">
    <SEOHead title="Turbine Services — Overhauling, Erection & Reverse Engineering India" description="Expert steam turbine overhauling, erection & commissioning, precision reverse engineering, dynamic balancing ISO 1940/API 670, lube oil flushing ISO 4406:99, and machine alignment for turbines 5 kW to 27 MW across all major OEMs."
...
const IndustriesPage = ({navigate}) => (
  <main id="main-content" className="pt-24 pb-20 bg-slate-50 min-h-screen">
    <SEOHead title="Industries Served — Power Plants, Sugar Mills, Oil & Gas, Petrochemical India" description="Keshav Enterprises serves power generation plants, sugar mills, paper mills, oil & gas, petrochemical refineries and agro industries with turbine engineering, OEM-compatible spares, and industrial products across India."
...
const BlogPage = ({navigate}) => (
  <main id="main-content" className="pt-24 pb-20 bg-slate-50 min-h-screen">
    <SEOHead title="Engineering Blog — Turbine Maintenance, Reverse Engineering & Industrial Insights" description="Technical articles on steam turbine overhauling, lube oil contamination, reverse engineering turbine spares in India, maintenance checklists and industrial best practices from Keshav Enterprises engineering team."
...
const ContactPage = () => {
  ...
  return (
    <main id="main-content" className="pb-20 bg-slate-50 min-h-screen">
      <SEOHead title="Contact Engineering Team — Request a Technical Quote" description="Contact Keshav Enterprises for turbine engineering RFQs, reverse engineering quotes, OEM spare parts, and 24x7 emergency breakdown support. Call +91 9149229448 or WhatsApp us. Shamli, Uttar Pradesh, India."
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    /* ── Suppress browser-extension noise in console ──
       "A listener indicated an asynchronous response by returning true,
        but the message channel closed before a response was received"
       This is a Chrome extension bug (Grammarly / Translate / etc.), not our code.
       We filter it so it doesn't alarm developers inspecting the console. */
    ...
    // CSP meta — defence-in-depth layer (server header always takes precedence)
    if(!document.querySelector('meta[http-equiv="Content-Security-Policy"]')){
      ...
      csp.content=[
        ...
        "connect-src 'self' https://api.whatsapp.com https://maps.googleapis.com",
        // Allow Google Maps iframe on contact page (was 'none' — blocked the map)
    ...
    const hints = [
      ...
      // Use rel=stylesheet (not preload as=style) — preload without a matching stylesheet
      // triggers "preloaded but not used" warnings and wastes a network slot
      {rel:'stylesheet',  href:'https://fonts.googleapis.com/css2?family=Barlow:ital,wght@0,500;0,700;0,900;1,400&family=Barlow+Condensed:wght@700;900&display=swap'},
    ];
    ...
    /* Initial ready — use rAF instead of setTimeout to avoid async channel leaks */
    let rafId = requestAnimationFrame(()=>{
      rafId = requestAnimationFrame(()=>setAppReady(true));
    });
    return()=>{window.removeEventListener('popstate',h);cancelAnimationFrame(rafId);};
  },[]);

  const navigate=useCallback((path)=>{
    pendingPath.current=path;
    setIsTransitioning(true);
    // Use rAF chain instead of setTimeout — avoids holding async message channels open
  ...
  /* Per-route skeleton — shown during transitions */
  const transitionSkeleton=useMemo(()=>{
    const p=pendingPath.current||currentPath;
    if(p.startsWith('/product/'))  return <ProductDetailSkeleton/>;
    if(p.startsWith('/blog/'))     return <BlogPostSkeleton/>;
    switch(p){
      case '/':           return <HomePageSkeleton/>;
      case '/about':      return <AboutPageSkeleton/>;
      case '/blog':
      case '/blog/':      return <BlogPageSkeleton/>;
      case '/services':   return <ServicesPageSkeleton/>;
      case '/products':   return <ProductsPageSkeleton/>;
      case '/industries': return <IndustriesPageSkeleton/>;
      case '/contact':    return <ContactPageSkeleton/>;
  ...
  /* Initial full-page skeleton — shown for first 400ms while fonts/CSS load */
  if(!appReady) return (
    <div className="font-sans min-h-screen flex flex-col bg-[#0A1628] overflow-x-hidden">
      <style>{GLOBAL_CSS}{SKELETON_CSS}</style>
      {/* Skeleton navbar */}
      <div className="fixed w-full z-50 py-5" aria-hidden="true">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 ske-dark rounded-lg"/>
            <div className="w-36 h-5 ske-dark rounded"/>
          </div>
          <div className="hidden lg:flex gap-2 items-center">
            {[60,50,70,60,75,55].map((w,i)=>(
              <div key={i} className={`h-7 ske-dark rounded-lg ske-delay-${Math.min(i+1,6)}`} style={{width:w}}/>
            ))}
            <div className="w-24 h-9 ske-dark rounded-lg ml-3 ske-delay-6"/>
          </div>
          <div className="lg:hidden w-8 h-8 ske-dark rounded"/>
        </div>
      </div>
      {transitionSkeleton}
    </div>
  ...
  return (
    <div className="font-sans min-h-screen flex flex-col bg-white selection:bg-blue-600 selection:text-white text-[#111827] overflow-x-hidden">
      ...
      <div className="flex-1 flex flex-col">
        {isTransitioning ? transitionSkeleton : (
          <div className="ske-page-enter flex-1 flex flex-col">{page}</div>
        )}
      </div>
